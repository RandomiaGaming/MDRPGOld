...
public class Player : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        float Degree = Mathf.Rad2Deg * Mathf.Atan2((transform.position - Camera.main.ScreenToWorldPoint(Input.mousePosition)).y, (transform.position - Camera.main.ScreenToWorldPoint(Input.mousePosition)).x);
        if (Degree > 180)
        {
            Degree -= 180;
            Degree = 180 - Degree;
